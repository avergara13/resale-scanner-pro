...
export function SessionLiveBanner() {
  ...
  return (
    <div style={barStyle} className="flex items-center gap-2 px-3 py-1.5">
      {/* Sonar ping dot */}
      ...
      {/* LIVE label */}
      <span className="text-[8px] font-black text-green tracking-[0.15em] uppercase flex-shrink-0">LIVE</span>

      {/* Divider */}
      <span className="w-px h-3 bg-green/25 flex-shrink-0" />

      {/* Session name */}
      <span className="text-[10px] font-bold text-green font-mono flex-shrink-0 max-w-[90px] truncate">
        {sessionLabel}
      </span>

      {/* Operator name */}
      {currentSession.operatorName && (
        <span className="text-[9px] text-t3 truncate flex-1 min-w-0">{currentSession.operatorName}</span>
      )}

      {/* Stats */}
      <div className="flex items-center gap-1 flex-shrink-0 ml-auto">
        <span className="text-[9px] font-bold text-green">{currentSession.buyCount}</span>
        <span className="text-[9px] text-t3 font-medium">buy</span>
        <span className="text-[9px] text-s3 mx-0.5">·</span>
        <span className="text-[9px] font-bold text-red">{currentSession.passCount}</span>
        <span className="text-[9px] text-t3 font-medium">pass</span>
        <span className="text-[9px] text-s3 mx-0.5">·</span>
        <span className="text-[9px] font-bold text-amber">{maybeCount}</span>
        <span className="text-[9px] text-t3 font-medium">maybe</span>
      </div>

      {/* Collapse */}
      <button
        onClick={() => setCollapsed(true)}
        className="flex-shrink-0 text-green/60 hover:text-green transition-colors ml-2"
